;(function(exports, undefined) {
  ...
  Facet.makeLayers = function(spec) {
    return _.map(spec.layers, function(s) {
      return new Layer(s)
    })
  }
  ...
  SingleFacet.prototype.subrender = function(
    x,
    y,
    width,
    height,
  ...
  XYFacet.prototype.subrender = function(x, y, width, height, paddingX, paddingY, svg, data) {
    var xs = _.sortBy(_.uniq(_.pluck(data, this.x)), function(x) {
      return x
    })
    var ys = _.sortBy(_.uniq(_.pluck(data, this.y)), function(y) {
      return y
    })
    var grouped = _.mapObject(
      _.groupBy(data, this.x),
      _.bind(function(ds) {
        return _.groupBy(ds, this.y)
      }, this)
    )
    ...
    _.each(ys, function(ylabel, yindex) {
      ...
      svg
        .append('g')
        .attr('transform', translate(xcoord + labelWidth * 0.75, ycoord) + ' rotate(270)')
        .append('text')
        .attr('class', 'y facet label')
        .text(ylabel)
        .attr('text-anchor', 'middle')
  ...
  function makeScales(specs, layers, aesthetics, data, width, height, paddingX, paddingY) {
    var scaleSpecs = _.object(
      _.map(specs, function(s) {
        return [s.aesthetic, s]
      })
    )

    // Get all possible values for aesthetic a.
    function allValues(a) {
      function hasAesthetic(layer) {
        return a in layer.mappings
      }
      function vals(layer) {
        function v(d) {
          return layer.dataValues(d, a)
        }
  ...
  function Layer(spec) {
    ...
    this.mappings = _.object(
      _.without(
        _.map(
          this.geometry.aesthetics,
          function(a) {
            if (a in spec) {
  ...
  Layer.prototype.attributeValue = function(scales, aesthetic, defaultValue) {
    return this.mappings[aesthetic]
      ? _.bind(function(d) {
          return this.aestheticValue(scales, d, aesthetic)
        }, this)
      : defaultValue
  }
  ...
  PointGeometry.prototype.render = function(g, data, layer, scales) {
    ...
    groups(g, 'circles', data)
      ...
      .attr('cy', function(d) {
        return layer.aestheticValue(scales, d, 'y')
      })
      .attr('fill-opacity', 1)
  ...
  AreaGeometry.prototype.valuesForAesthetic = function(datum, field, layer) {
    return field
      ? [datum[field]]
      : _.map(['y0', 'y1'], function(x) {
          return layer.dataValue(datum, x)
        })
  }
  ...
  LineGeometry.prototype.render = function(g, data, layer, scales) {
    function scale(d, aesthetic) {
      return layer.aestheticValue(scales, d, aesthetic)
    }
    ...
    var color = layer.mappings.color
      ? function(d) {
          return scale(d[0], 'color')
        }
      : this.color
  ...
  IntervalGeometry.prototype.render = function(g, data, layer, scales) {
    ...
    groups(g, 'rects', data)
      ...
      .attr('height', function(d) {
        return layer.scaledMin('y', scales) - scale(d, 'y')
      })
      .attr('fill', layer.attributeValue(scales, 'color', this.color))
  }
  ...
  BoxPlotGeometry.prototype.render = function(g, data, layer, scales) {
    ...
    function iqrBox(s) {
      s.append('rect')
        ...
        .attr('x', function(d) {
          return scale(d.group, 'x') - width / 2
        })
        .attr('y', function(d) {
          return scale(d.q3, 'y')
        })
    ...
    function whiskerTick(s, y) {
      s.append('line')
        .attr('class', 'boxplot whisker')
        .attr('x1', function(d) {
          return scale(d.group, 'x') - width * 0.4
        })
        .attr('x2', function(d) {
          return scale(d.group, 'x') + width * 0.4
        })
    ...
    function outliers(s) {
      s.selectAll('circle')
        .data(function(d) {
          return _.map(d.outliers, function(o) {
            return { x: scale(d.group, 'x'), y: scale(o, 'y') }
          })
        })
        .enter()
        .append('circle')
    ...
    function render(s) {
      s.call(iqrBox)
        .call(whisker, 'q3', 'upper')
        .call(whisker, 'q1', 'lower')
        .call(outliers)
    }

    var color =
      'color' in layer.mappings
        ? function(d) {
  ...
  ArrowGeometry.prototype.render = function(g, data, layer, scales) {
    ...
    function arrowline(s) {
      s.append('line')
        .attr('x1', function(d) {
          return scale(d.tail.x, 'x')
        })
        .attr('x2', function(d) {
          return scale(d.head.x, 'x')
        })
        .attr('y1', function(d) {
          return scale(d.tail.y, 'y')
        })
        .attr('y2', function(d) {
          return scale(d.head.y, 'y')
        })
        .attr('fill', 'none')
    ...
    function arrowhead(s) {
      ...
      s.append('svg:path')
        .attr('class', 'arrow')
        .attr('d', function(d) {
          return line(arrowheadPoints(d, len, width)) + 'Z'
        })
        .attr('fill', color)
        .attr('stroke-width', linewidth)
        .attr('stroke', color)
  ...
  TextGeometry.prototype.render = function(g, data, layer, scales) {
    ...
    var area = g.append('g')
    var text = groups(area, 'texts', data)
      .selectAll('circle')
      .data(Object)
      .enter()
      .append('text')
  ...
  Scale.prototype.defaultDomain = function(values) {
    ...
    this.domain(
      this.center !== undefined ? centered(this.min, this.max, this.center) : [this.min, this.max]
    )
    this.domainSet = true
  }
  ...
  IdentityStatistic.prototype.compute = function(data) {
    return data
  }
  ...
  SumStatistic.prototype.compute = function(data) {
    ...
    var value = _.bind(function(point) {
  ...
  BoxPlotStatistic.prototype.dataRange = function(data) {
    ...
    return [_.min(_.pluck(flattened, 'min')), _.max(_.pluck(flattened, 'max'))]
  }
  ...
  BoxPlotStatistic.prototype.compute = function(data) {
    ...
    return _.map(
      _.sortBy(_.pairs(groups), function(p) {
        return ordering(p[0])
      }),
      function(g) {
        var name = g[0]
  ...
  exports.gg = function gg() {
    var graphic = new Graphic({
      facets: _.find(arguments, function(x) {
        return _.has(x, 'facets')
      }),
      layers: _.filter(arguments, function(x) {
        return _.has(x, 'geometry')
      }),
      scales: _.filter(arguments, function(x) {
        return _.has(x, 'aesthetic')
      }),
    })
    return function(data, where, opts) {
      graphic.render(data, where, opts)
    }
  }
})(this)
